    ...
    <div class="container-fluid">
      <div class="row">
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
          <div class="position-sticky pt-3">
            <ul class="nav flex-column">
              ...
              <li class="nav-item">
                <a class="nav-link faculty" aria-current="page" href="addfaculty.php">
              ...
              <li class="nav-item">
                <a class="nav-link faculty" aria-current="page" href="totatt.php">
                  <span data-feather="users"></span>
                  Total Attendance
                </a>
              </li>
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered shadow-sm align-middle">
                            ...
                            <tbody>
                                ...
                                $query = "select * from proctoring_student";
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    $proctoring_student_semester = $row['proctoring_student_semester'];
                                    $proctoring_student_department = $row['proctoring_student_department'];
                                    $proctoring_student_email = $row['proctoring_student_email'];
                                    $proctoring_student_phone = $row['proctoring_student_phone'];
                                    $proctoring_student_gender = $row['proctoring_student_gender'];

                                    echo '<tr class="text-center">';
                                    echo '<td>' . $proctoring_student_name . '</td>';
                                    echo '<td>' . $proctoring_student_usn . '</td>';
                                    echo '<td>' . $proctoring_student_email . '</td>';
                                    echo '<td>' . $proctoring_student_gender . '</td>';
                                    //echo '<td>' . $proctoring_student_semester . '</td>';
                                    //echo '<td>' . $proctoring_student_department . '</td>';
                                    echo '<td>' . $proctoring_student_phone . '</td>';
          ...
          <form id="form" name="form" method="POST" action="facultydelete.php" class="text-center" enctype="multipart/form-data">
            ...
            <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered shadow-sm align-middle">
                            ...
                            <tbody>
                                <?php
                                // $email = $_SESSION['user_email'];
                                $query = "select * from proctoring_faculty";

                                $result = mysqli_query($link, $query);

                                while ($row = mysqli_fetch_array($result)) {
                                    $id = $row['id'];
                                    $proctoring_faculty_name = $row['proctoring_faculty_name'];
                                    $proctoring_faculty_fid = $row['proctoring_faculty_fid'];
                                    $proctoring_faculty_semester = $row['proctoring_faculty_semester'];
                                    $proctoring_faculty_department = $row['proctoring_faculty_department'];
                                    $proctoring_faculty_subject = $row['proctoring_faculty_subject'];
                                    $proctoring_faculty_email = $row['proctoring_faculty_email'];
                                    $proctoring_faculty_phone = $row['proctoring_faculty_phone'];
                                    $proctoring_faculty_gender = $row['proctoring_faculty_gender'];

                                    echo '<tr class="text-center">';
                                    echo '<td>' . $proctoring_faculty_name . '</td>';
                                    echo '<td>' . $proctoring_faculty_fid . '</td>';
                                    echo '<td>' . $proctoring_faculty_email . '</td>';
                                    echo '<td>' . $proctoring_faculty_gender . '</td>';
                                    //echo '<td>' . $proctoring_faculty_semester . '</td>';
                                    //echo '<td>' . $proctoring_faculty_department . '</td>';
                                    //echo '<td>' . $proctoring_faculty_subject . '</td>';
                                    echo '<td>' . $proctoring_faculty_phone . '</td>';
                                    echo '<td><div class="form-group"><button class="btn btn-danger btn-sm delete text-white" id ="butdel" name ="butdel" value="' . $id . '"><i class="fa fa-trash"></i></button></div></td>';
                                    echo '</tr>';
                                }
                                ?>
