      ...
      <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h3 lead">Assign Proctor</h1>
          </div>
          <?php
          ...
          ?>
              <div class="alert alert-success alert-dismissible fade show mt-2 text-capitalize" role="alert">
                <strong>Proctor Assigned Successfully</strong>
            ...
            ?>
              <div class="alert alert-danger alert-dismissible fade show mt-2 text-capitalize" role="alert">
                <strong> Proctor Assign Failed</strong>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
              </div>
          <?php
            }
          ...
          <form id="form" name="form" method="GET" action="assigprocdata.php" class="text-center">
		  <div class="mb-3 col-md-4">
                <label class="form-label">Select Proctor :</label>
                <select class="form-select shadow-none" name="proctor" id="proctor" required="required" data-placeholder='--Select Subject--'>
                  <option>Select Faculty</option>
				   <?php
                                // $email = $_SESSION['user_email'];
                                $query = "select proctoring_faculty_name from proctoring_faculty";

                                $result = mysqli_query($link, $query);

                                while ($row = mysqli_fetch_array($result)) {
									 echo '<option>' . $row['proctoring_faculty_name'] . '</option>';
                                }
				?>
              ...
              <div class="mb-3 col-md-4">
                <input type="submit" class="btn w-100 text-light" value="Assign" name="butsave" id="butsave" style="background: #FF970A;" />
              </div>
            </div>
            ...
            <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered shadow-sm align-middle">
                            ...
                            <tbody>
                                <?php
                                $query = "select * from proctoring_student where Proctor='--'";
                                ...
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    echo '<td><input type="checkbox" value="'.$id.'" onchange="checkCheckbox('.$id.')" /></td>';
						...
						<input type="hidden" id="selcheck" name="selcheck" />
						<script>
							function checkCheckbox(id)
							{
								//alert(id);
								var selcheckval=document.getElementById('selcheck').value;
								selcheckval=selcheckval+id+",";
								//alert(selcheckval);
								document.getElementById('selcheck').value=selcheckval;
							}
						</script>
                    </div>
          </form>
